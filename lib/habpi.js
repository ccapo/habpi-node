...
module.exports.HABPi = class HABPi {
  constructor(options) {
    ...
    this.db = new Sequelize('habpi', null, null, {
      ...
      isolationLevel: Sequelize.Transaction.SERIALIZABLE, // READ_UNCOMMITTED, READ_COMMITTED, REPEATABLE_READ, SERIALIZABLE
      retry: {max: 5, match: ['SQLITE_BUSY: database is locked', Sequelize.ConnectionError]},
      benchmark: false
    });
  ...
  initBroadcast() {
    ...
    this.broadCastTimer = setInterval(() => {
      self.log(`Broadcast Sensor Payload: ${JSON.stringify(self.payload)}`, 'alert');
    }, BROADCAST_INTERVAL);
  }
