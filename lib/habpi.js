  ...
  init() {
    ...
    this.board.on('ready', function() {
      ...
      mpl.on('change', function() {
        ...
        self._sequelize.models.sensor_type.findAll().then(sensorTypes => {
          for(let sensorType of sensorTypes) {
            self.log(`${sensorType.dataValues.name}: ${sensorType.dataValues.id}`, 'warning');
          }
          self.log(Constants.SENSOR_TYPE);
        });
        /*self.db.models.message_type.create({
          sensor_type_id: 1,
          message_type_id: 2,
          message: 
        });*/
