...
module.exports.HABPi = class HABPi {
  constructor(options) {
    ...
    this._sequelize = new Sequelize('habpi', null, null, {
      ...
      logging: console.log,
      storage: './db/habpi.sqlite', // ':memory:',
  ...
  init() {
    ...
    this._sequelize.authenticate().then(() => {
      self.log('Connection has been established successfully', 'alert');
      self._loadModels();
    }).catch(err => {
      self.log(`Unable to connect to the database: ${JSON.stringify(err)}`, 'error');
    });
    ...
    this.board.on('ready', function() {
      ...
      mpl.on('change', function() {
        ...
        // Log sensor data
        self.log(`MPL3115A2: Temperature, Pressure, Altitude`, 'notice');
        self.log(`${this.thermometer.celsius} C, ${this.barometer.pressure} kPa, ${this.altimeter.meters} m`);
      });
      ...
      bmp.on("change", function() {
        ...
        // Log sensor data
        self.log(`BMP180: Temperature, Pressure, Altitude`, 'notice');
        self.log(`${this.thermometer.celsius} C, ${this.barometer.pressure} kPa, ${this.altimeter.meters} m`);
      });
  ...
  // Load models
  _loadModels() {
    let baseDir = path.join(__dirname, '/lib/models/');
    let files = fs.readdirSync(baseDir);
    for (const file of files) {
      if (fs.lstatSync(path.join(baseDir, file)).isFile()) {
        this._sequelize.import(__dirname + '/lib/models/' + file);
      }
    }
