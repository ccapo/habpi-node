  ...
  init() {
    ...
    this.board.on('ready', function() {
      ...
      mpl.on('change', function() {
        ...
        // Log sensor data
        self.log(`MPL3115A2: ${this.thermometer.celsius} C, ${this.barometer.pressure} kPa, ${this.altimeter.meters} m`, 'error');
        self.db.models.message.create({
          sensor_type_id: Constants.SENSOR_TYPE.MPL3115A2,
          message_type_id: Constants.MESSAGE_TYPE.TEMP,
          message: this.thermometer.celsius
        }).then(() => {
          return self.db.models.message.create({
            sensor_type_id: Constants.SENSOR_TYPE.MPL3115A2,
            message_type_id: Constants.MESSAGE_TYPE.PRES,
            message: this.barometer.pressure
          });
        }).then(() => {
          return self.db.models.message.create({
            sensor_type_id: Constants.SENSOR_TYPE.MPL3115A2,
            message_type_id: Constants.MESSAGE_TYPE.ALT,
            message: this.altimeter.meters
          });
        }).catch(err => {
